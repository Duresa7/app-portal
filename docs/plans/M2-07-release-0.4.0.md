...
- Migration from zip installs: document that the last zip-based release's updater will not install an MSI; 0.3.x devices are moved by deploying the 0.4.0 MSI once through the same channel the zip went through. The MSI removes the scheduled task and reuses `client.json`.
...
- Tag run green, release carries `AppPortal-0.4.0-x64.msi`, `AppPortalSetup.exe`, `SHA256SUMS`, and the server image is pushed.
