...
2. **One package, one branch, one pull request.** Branch from `main` as `plan/<id>`, for example `plan/M1-04`. The pull request gate must be green: format, build, tests and the server image, all on Linux. The Windows jobs cost several times as much per minute and no longer run per commit; run the full gate from the Actions tab before a release, and fix what it finds before the tag. A human merges.
